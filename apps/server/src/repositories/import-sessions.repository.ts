...
export interface ImportSessionsRepository {
	cleanupOldSessions(
		daysOld?: number,
	): Promise<{ sessionsDeleted: number; linksDeleted: number }>;
	countBySession(
		sessionId: string,
		userId: string,
	): Promise<{
	...
	updateStatus(
		...
		updates: Partial<
			Pick<
				ImportSessionData,
				"completedAt" | "errorMessage" | "startedAt" | "status"
			>
		>,
	): Promise<ImportSessionData | null>;
}
...
export function createDrizzleImportSessionsAdapter(
	db: DrizzleClient,
		...
		const [row] = await db
			...
			.where(
				and(
					...
					eq(importSessionsTable.userId, userId),
				),
			)
			.limit(1);
	...
	async function incrementExtractionCounts(
		...
		if (row) {
			...
			const totalProcessed =
				session.extractionCompleted + session.extractionFailed;
			const progress =
				session.importedCount > 0
					? Math.round((totalProcessed / session.importedCount) * 100)
					: 0;
	...
	return {
		...
		async findByUserId(userId, options) {
			...
			if (statusFilter && statusFilter !== "all") {
				conditions.push(
					eq(
						importSessionsTable.status,
						statusFilter as ImportSessionData["status"],
					),
				);
			}
			...
			if (cursor) {
				try {
					...
					conditions.push(
						lt(importSessionsTable.createdAt, new Date(cursorData.createdAt)),
					);
				} catch (error) {
					logger.error(
						`Invalid cursor provided for import sessions: ${cursor} ${error}`,
					);
			...
			const { items, hasMore } = pageRows(rows, limit);			const nextCursor = hasMore
				? extractCursor(
						items.map((item) => ({
		...
		async findLinksBySession(sessionId, userId, options) {
			...
			if (statusFilter && statusFilter !== "all") {
				conditions.push(
					eq(
						linksTable.processingStatus,
						statusFilter as LinkWithStatus["processingStatus"],
					),
				);
			}
			...
			if (cursor) {
				try {
					...
					logger.error(
						`Invalid cursor provided for session links: ${cursor} ${error}`,
					);
					throw new Error("Invalid cursor format");
				}
			}
			...
			const links = resultRows.map((row) => ({
				...
				url: row.url,
				processingStatus:
					row.processingStatus as LinkWithStatus["processingStatus"],
		...
		async countBySession(sessionId, userId) {
			...
			return {
				...
				completed: allRows.filter((r) => r.processingStatus === "completed")
					.length,
		...
		async cleanupOldSessions(daysOld = 90) {
			...
			if (oldSessionIds.length === 0)
				return { sessionsDeleted: 0, linksDeleted: 0 };
			...
			for (const sessionId of oldSessionIds) {
				...
				if (linkIds.length > 0) {
					...
					await db
						...
						.where(inArray(linkTagsTable.linkId, linkIdList));
