...
async function readCsvFile(
	...
	const headerColumns = parseLine(lines[0] as string).map((col) =>
...
async function csvImportJob(job: Job<CsvImportJobData>): Promise<{
	...
	logger.info(
		`[${workerName}] Starting CSV import job ${job.id} for session ${importSessionId}`,
	);
	...
	try {
		...
		const existingUrls = new Set(
			(await links.findAllUrls(userId)).map((url) => url.toLowerCase()),
		);
		...
		const session = await importSessions.findByIdOrThrow(
			importSessionId,
			userId,
		);
		...
		for (let i = 0; i < dataLines.length; i += batchSize) {
			...
			for (const line of batch) {
				try {
					...
					const urlIndex = headerColumns.indexOf(
						fieldMapping.url?.toLowerCase() || "url",
					);
					const titleIndex = headerColumns.indexOf(
						fieldMapping.title?.toLowerCase() || "title",
					);
					const tagsIndex = headerColumns.indexOf(
						fieldMapping.tags?.toLowerCase() || "tags",
					);
					...
					const title =
						titleIndex >= 0 ? columns[titleIndex] : columns[1] || url || "";
					const tagsValue = tagsIndex >= 0 ? (columns[tagsIndex] ?? "") : "";
					const timeAddedValue =
						timeAddedIndex >= 0 ? Number(columns[timeAddedIndex]) : new Date();
		...
		if (pendingLinks.length > 0) {
			...
			logger.info(
				`[${workerName}] Enqueued first link ${firstLink?.id} for content extraction`,
			);
		} else {
			await importSessions.updateExtractionStatus(importSessionId, userId, {
		...
		try {
			...
			logger.error(
				`[${workerName}] Failed to update session status: ${updateError}`,
			);
		}
...
csvImportWorker.on("completed", (job: Job<CsvImportJobData>, result) => {
	logger.info(
		`CSV import job ${job.id} completed. Result: ${JSON.stringify(result)}`,
	);
});

csvImportWorker.on(
	"failed",
	(job: Job<CsvImportJobData> | undefined, error) => {
...
csvImportWorker.on(
	"progress",
	(job: Job<CsvImportJobData>, progress: JobProgress) => {
...
csvImportWorker.on("error", (error: Error) => {
	logger.error(
		`[${workerName}] Error in CSV import worker: ${JSON.stringify(error)}`,
	);
});
