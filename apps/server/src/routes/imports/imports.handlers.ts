...
export const previewImport: AppRouteHandler<PreviewImportRoute> = async (c) => {
	...
	try {
		...
		const previewRows = dataLines.slice(0, 5).map((line) => {
			const columns = parseLine(line).map((c) =>
				c.trim().replaceAll(/^"|"$/g, ""),
			);
			const headerColumns = hasHeader
				? firstLine
						.split(",")
						.map((c) => c.trim().replaceAll(/^"|"$/g, "").toLowerCase())
				: [];

			const urlColIndex = mapping?.url
				? headerColumns.indexOf(mapping.url.toLowerCase())
				: headerColumns.findIndex(
						(c) => c === "url" || c === "href" || c === "link",
					);
			...
			const timeAddedColIndex = mapping?.timeAdded
				...
				: headerColumns.findIndex(
						(c) => c === "time_added" || c === "added_at",
					);

			const url = urlColIndex >= 0 ? columns[urlColIndex] : columns[0];
			const title =
				titleColIndex >= 0 ? columns[titleColIndex] : columns[1] || "";
			const tagsStr = tagsColIndex >= 0 ? columns[tagsColIndex] : "";
			const timeAddedStr =
				timeAddedColIndex >= 0 ? columns[timeAddedColIndex] : "";
...
export const executeImport: AppRouteHandler<ExecuteImportRoute> = async (c) => {
	...
	try {
		...
		const baseExists = existingGroups.some(
			(group) => group.name === baseTagName,
		);
		...
		if (baseExists) {
			...
			for (const group of existingGroups) {
				...
				if (match)
					maxNum = Math.max(maxNum, Number.parseInt(match[1] ?? "0") + 1);
			}
...
export const listImportSessions: AppRouteHandler<
	ListImportSessionsRoute
> = async (c) => {
	const user = c.get("user");
...
export const getImportSession: AppRouteHandler<GetImportSessionRoute> = async (
	c,
) => {
	const user = c.get("user");
...
export const cancelImportSession: AppRouteHandler<
	CancelImportSessionRoute
> = async (c) => {
	const user = c.get("user");
	const { id } = c.req.valid("param");
	...
	try {
		...
		const linksReset = await importSessions.resetProcessingLinksForCancel(
			id,
			user.id,
		);
...
export const deleteImportSession: AppRouteHandler<
	DeleteImportSessionRoute
> = async (c) => {
	const user = c.get("user");
	const { id } = c.req.valid("param");
...
export const resumeImport: AppRouteHandler<ResumeImportRoute> = async (c) => {
	...
	try {
		...
		const pendingLinks = await importSessions.findPendingLinksInSession(
			id,
			user.id,
			1,
		);
...
export const getSessionLinks: AppRouteHandler<GetSessionLinksRoute> = async (
	c,
) => {
	const user = c.get("user");
	const { id } = c.req.valid("param");
...
export const retryFailedImport: AppRouteHandler<
	RetryFailedImportRoute
> = async (c) => {
	const user = c.get("user");
	const { id } = c.req.valid("param");
		...
		try {
			const extractionProgress =
				session.importedCount > 0
					? Math.round(
							(session.extractionCompleted / session.importedCount) * 100,
						)
					: 0;
			...
			logger.error(
				`[import] Failed to update extraction status for session ${id}: ${updateError}`,
			);
		}
...
async function cleanupOrphanedTempFiles(): Promise<number> {
	...
	try {
		...
		const csvFiles = files.filter(
			(f) => f.startsWith("import_") && f.endsWith(".csv"),
		);
...
export const cleanupOldSessions: AppRouteHandler<
	CleanupOldSessionsRoute
> = async (c) => {
	const user = c.get("user");
	const { importSessions } = c.get("repos");
